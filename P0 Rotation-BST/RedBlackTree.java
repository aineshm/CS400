...
public class RedBlackTree<T extends Comparable<T>> implements SortedCollectionInterface<T> {
	...
	/**
	 * Tester method - tests for IllegalArgumentException throwing Tests one invalid
	 * and one valid rotate method call
	 * 
	 * @return true if exception is caught for an invalid
	 */
	public static boolean test1() {
		try {
			...
			// check for invalid
			try {
				bst.rotate(bst.findNodeWithData(3), bst.findNodeWithData(34));
			} catch (IllegalArgumentException e) {
				// does nothing
			}

			try {
				bst.rotate(bst.findNodeWithData(14), bst.findNodeWithData(25));
			} catch (IllegalArgumentException e) {
				return false;
			}
	...
	/**
	 * Tester method - tests 2 valid rotate arguments in 2 cases Test1 - tests a
	 * right rotate at a left subtree Test2 - tests a left rotate at a left subtree
	 * 
	 * Both are compared to in order string as well as the level order strings to
	 * make sure the rotate method works
	 * 
	...
	public static boolean test2() {
		...
		try {
			// creating bst objecct and inserting numbers
			RedBlackTree bst = new RedBlackTree();
			bst.insert(34);
			bst.insert(25);
			bst.insert(14);
			...
			// testing right rotate on a set of nodes (parent not at root)
			try {
				bst.rotate(bst.findNodeWithData(14), bst.findNodeWithData(25));
				if (!bst.toInOrderString().equals("[ 3, 14, 25, 30, 34, 44, 45, 46, 78, 99 ]")) {
					System.out.println("Error - Order not preserved");
					return false;
				}
				if (!bst.toLevelOrderString().equals("[ 34, 14, 78, 3, 25, 45, 99, 30, 44, 46 ]")) {
					System.out.println("Error in level order string");
					return false;
				}
			} catch (IllegalArgumentException e) {
				return false;
			...
			// testing left rotate on a set of nodes (parent not at root) - reversing right
			// rotate above
			try {
				bst.rotate(bst.findNodeWithData(25), bst.findNodeWithData(14));
				if (!bst.toInOrderString().equals("[ 3, 14, 25, 30, 34, 44, 45, 46, 78, 99 ]")) {
					System.out.println("Error - Order not preserved");
					return false;
				}
				if (!bst.toLevelOrderString().equals("[ 34, 25, 78, 14, 30, 45, 99, 3, 44, 46 ]")) {
					System.out.println("Error in level order string");
					return false;
				}
			} catch (IllegalArgumentException e) {
		...
		} catch (Exception e) { // checks for any other exception that may be thrown, which means that rotate()
								// doesn't work
			return false;
		}
	...
	/**
	 * Tester method - tests for a left and right rotate at the root node
	 * 
	...
	public static boolean test3() {
		try {
			...
			// testing right rotate on a set of nodes at the top of the bst
			try {
				bst.rotate(bst.findNodeWithData(25), bst.findNodeWithData(34));
				if (!bst.toInOrderString().equals("[ 3, 14, 25, 30, 34, 44, 45, 46, 78, 99 ]")) {
					System.out.println("Error - Order not preserved");
					return false;
				}
				if (!bst.toLevelOrderString().equals("[ 25, 14, 34, 3, 30, 78, 45, 99, 44, 46 ]")) {
					System.out.println("Error in level order string");
					return false;
				}
			} catch (IllegalArgumentException e) {
			...
			// testing left rotate on a set of nodes at the top of the bst - reversing right
			// rotate above
			try {
				bst.rotate(bst.findNodeWithData(34), bst.findNodeWithData(25));
				if (!bst.toInOrderString().equals("[ 3, 14, 25, 30, 34, 44, 45, 46, 78, 99 ]")) {
					System.out.println("Error - Order not preserved");
					return false;
				}
